...
import CategoriesContainer from "@/components/categories_container";
import Categories from "@/components/categories.jsx";
import DuaContainer from "@/components/dua_container";
import CategoryList from "@/components/catogory_list";
import DetailsContainer from "@/components/DetailsContainer";
import { getCategory, getSubcat, getDua } from "@/utils/fetch-data";
...
export default function Home({ category, subcat, dua }) {
  return (
    <div
      style={{
        overflow: "auto",
      }}
      className="font-inter style-en false z-0 lg:fixed lg:inset-x-0 
                xl:fixed xl:inset-x-0 xl:p-6 xl:pb-0 2xl:px-10 2xl:pt-6 2xl:fixed 
                2xl:inset-x-0 3xl:p-10 3xl:pb-0 3xl:fixed 3xl:inset-x-0"
    >
      <Container>
        <Sidebar />
        <Navbar />
        <CategoriesContainer>
          <Categories />
          <DuaContainer>
            <CategoryList category={category} subcat={subcat} dua={dua} />
          </DuaContainer>
          <DetailsContainer dua={dua} />
        </CategoriesContainer>
      </Container>
    </div>
  );
